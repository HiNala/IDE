...
| **M14** | Sidebar, tabs, quick open | **Done (code)** | `editor-ui::{Sidebar, TabStrip, QuickOpenPalette, CommandPalette}` painted via `FrameChrome`. `Ctrl+B` / `Ctrl+Shift+E` / `Ctrl+P` / `Ctrl+Shift+P` / `Ctrl+Tab` / `Ctrl+W` / `Ctrl+N` / `Ctrl+K Ctrl+O` wired. `Ctrl+K` parks a VS Code-style prefix (1.5 s timeout via `CHORD_TIMEOUT`); `Ctrl+K O` or `Ctrl+K Ctrl+O` opens a folder through `rfd::FileDialog::pick_folder()` and rebinds the workspace. Sidebar keyboard navigation (`Up/Down/Home/End/PageUp/PageDown`, `Left`/`Right` collapse/expand, `Enter/Space` activate, `Esc` defocus). Tab close X + dirty dot are rect-based icons. Breadcrumbs strip under the tab bar. Mouse routing respects chrome zones. Keyboard intercept while any palette visible. Sidebar width/visibility persist. `editor-app <dir>` auto-opens workspace. | Dirty-guard dialog on `Ctrl+W`; clickable breadcrumb navigation. |
