...
| **M14** | Sidebar, tabs, quick open | **Done (code)** | `editor-ui::{Sidebar, TabStrip, QuickOpenPalette}` painted via `FrameChrome`. `Ctrl+B` / `Ctrl+Shift+E` / `Ctrl+P` / `Ctrl+Tab` / `Ctrl+W` / `Ctrl+N` wired. Mouse routing respects chrome zones. Keyboard intercept while palette visible. Sidebar width/visibility persist. `editor-app <dir>` auto-opens workspace. | Up/Down/Enter navigation inside the sidebar tree; explicit `Ctrl+K Ctrl+O` keybinding for folder-open; dirty-guard dialog on `Ctrl+W`. |
| **M15** | Syntax highlighting | **Not started** | — | `tree-sitter` grammars, highlight layer in `editor-render`, theme tokens, language detection. No code in tree. |
...
| **M17** | Diff engine & renderer | **Library only** | `editor-diff`: `compute.rs` Myers diff + `intra_line.rs` + `session.rs` + `display.rs`. | Diff gutter marks + inline preview in `editor-render`; `editor-app` surface (ex: review sidebar, diff view mode). |
| **M18** | Git integration baseline | **Partial** | `editor-git::GitRepo::discover` + branch read via `gix`. Branch name in status bar, refreshed every 5s. | Modified-count in status bar, gutter change markers, commit/stage/diff/push/pull UI. |
